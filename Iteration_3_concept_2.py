...
custom_font = ("Rosewood Std Regular", "14")
...
# Function to make sure the same receipt number is never generated twice
...
# Get user data from refund or hire form inputs
def input_data():
    ...
    try:
        ...
        messagebox.showwarning("Warning!", "Please enter a valid input: amount must be between 1 and 500")
        return
...
def quit_confirmation():
    ...
    confirmation = tk.Label(quit_frame, text="Are you sure you want to quit?", fg='white', bg="#2A6543", font=("Cambria", 12, "bold"))
    confirmation.grid(row=0, column=0, columnspan=2, pady=10)

    # Yes and No buttons for quitting the program
    def quit_close():
        quit_window.destroy()
        main_window.destroy()
...
def display_receipt(receipt_number, first_name, last_name, item, amount):
    ...
    tk.Label(receipt_frame, text=f"Receipt Number: {receipt_number}", fg='white', bg='#2A6543', font=custom_font).pack(pady=5)
    tk.Label(receipt_frame, text=f"First Name: {first_name}", fg='white', bg='#2A6543', font=custom_font).pack(pady=5)
    tk.Label(receipt_frame, text=f"Last Name: {last_name}", fg='white', bg='#2A6543', font=custom_font).pack(pady=5)
    tk.Label(receipt_frame, text=f"Item Chosen: {item}", fg='white', bg='#2A6543', font=custom_font).pack(pady=5)
    tk.Label(receipt_frame, text=f"Amount: {amount}", fg='white', bg='#2A6543', font=custom_font).pack(pady=5)

    return_button = tk.Button(receipt_frame, text="Back", command=receipt_window.destroy, font=custom_font)
...
def hire_window():
    ...
    # Label frame which contains widgets inside
    user_info_frame = tk.LabelFrame(hire_frame, text="Hire Information", padx=80, pady=80, fg="white", bg="#2A6543", font=custom_font)
    ...
    # Sizing label and entry boxes
    label_width = 15
    entry_width = 20

    first_name_label = tk.Label(user_info_frame, text="First Name:", width=label_width, font=custom_font)
    ...
    first_name_entry = tk.Entry(user_info_frame, width=entry_width, validate="key", validatecommand=vcmd_first_name, font=custom_font)
    first_name_entry.grid(row=1, column=1, sticky="w")

    last_name_label = tk.Label(user_info_frame, text="Last Name:", width=label_width, font=custom_font)
    ...
    last_name_entry = tk.Entry(user_info_frame, width=entry_width, validate="key", validatecommand=vcmd_last_name, font=custom_font)
    last_name_entry.grid(row=2, column=1, sticky="w")

    item_label = tk.Label(user_info_frame, text="Item Chosen:", width=label_width, font=custom_font)
    item_label.grid(row=3, column=0, sticky="e")

    item_combobox = ttk.Combobox(user_info_frame, values=["Chairs", "Tables", "Benches", "Crockery"],
                                 width=entry_width - 2, state='readonly', font=custom_font)
    item_combobox.grid(row=3, column=1)

    amount_label = tk.Label(user_info_frame, text="Item Amount:", width=label_width, font=custom_font)
    amount_label.grid(row=4, column=0, sticky="e")

    amount_spinbox = tk.Spinbox(user_info_frame, from_=1, to=500, width=entry_width - 2, font=custom_font)
    amount_spinbox.grid(row=4, column=1)

    receipt_button = tk.Button(user_info_frame, text="Add To Order", command=input_data, padx=20, width=10, font=custom_font, **button_styling)
    receipt_button.grid(row=6, column=0, pady=10)

    back_button = tk.Button(user_info_frame, text="Back", command=hire_window.destroy, padx=22, width=10, font=custom_font, **button_styling)
...
def display_hired_items():
    ...
    display_window.geometry("900x290")
    ...
    for item in hired_items:
        tk.Label(display_frame,
                 ...
                 fg='white', bg='#2A6543', font=custom_font).pack(pady=5)

    return_button = tk.Button(display_frame, text="Close", command=display_window.destroy, font=custom_font, **button_styling)
    return_button.pack(pady=10)
...
def refund_section():
    ...
    refund_info_frame = tk.LabelFrame(refund_frame, text="Refund Information", padx=80, pady=80, bg='#2A6543', fg="white", font=custom_font)
    refund_info_frame.grid(row=0, column=0)

    label_width = 15
    entry_width = 20

    receipt_label = tk.Label(refund_info_frame, text="Receipt Number:", width=label_width, font=custom_font)
    receipt_label.grid(row=0, column=0, sticky="e")

    receipt_entry = tk.Entry(refund_info_frame, width=entry_width, font=custom_font)
    receipt_entry.grid(row=0, column=1)

    first_name_label = tk.Label(refund_info_frame, text="First Name:", width=label_width, font=custom_font)
    first_name_label.grid(row=1, column=0)

    first_name_entry = tk.Entry(refund_info_frame, width=entry_width, font=custom_font)
    first_name_entry.grid(row=1, column=1)

    item_label = tk.Label(refund_info_frame, text="Item:", width=label_width, font=custom_font)
    ...
    save_button = tk.Button(refund_info_frame, text="Process Refund", command=save_refund, padx=20, width=10, font=custom_font, **button_styling)
    save_button.grid(row=3, column=0, pady=10)

    display_receipt_button = tk.Button(refund_info_frame, text="Display Hired Items", command=display_hired_items, padx=20, width=12, font=custom_font, **button_styling)
    display_receipt_button.grid(row=3, column=1, pady=10)

    back_button = tk.Button(refund_info_frame, text="Back", command=refund_window.destroy, padx=22, width=10, font=custom_font, **button_styling)
...
text_styling = {"bg": "#2A6543", "fg": "white", "font": custom_font}

program_info = tk.Label(main_window, text="Welcome to Julies Party Hire!!", **text_styling)
program_info.pack(pady=10)

hire_button = tk.Button(main_window, text="Hire Item(s)", command=hire_window, **button_styling, font=custom_font, width=20)
hire_button.pack(pady=10)

refund_button = tk.Button(main_window, text="Return Item(s)", command=refund_section, **button_styling, font=custom_font, width=20)
refund_button.pack(pady=10)

display_button = tk.Button(main_window, text="Display Hired Item(s)", command=display_hired_items, **button_styling, font=custom_font, width=20)
display_button.pack(pady=10)

quit_button = tk.Button(main_window, text="Quit", command=quit_confirmation, **button_styling, font=custom_font, width=20)
