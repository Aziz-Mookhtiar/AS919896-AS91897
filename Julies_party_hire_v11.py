...
def input_data():
    ...
    #check if amount is less then 500 and greater then 1
    try:
        amount = int(amount)
...
#hire window
def hire_window():
    global first_name_entry, last_name_entry, item_combobox, amount_spinbox,image
    ...
    text_styling = {"bg": "#2A6543", "fg": "white", "font": ("Arial", 12, "bold")}

    hire_frame = tk.Frame(hire_window)
    hire_frame.pack()

    user_info_frame = tk.LabelFrame(hire_frame, text="Hire Information", padx=80, pady=80, fg="white", bg="#2A6543")
    ...
    amount_label = tk.Label(user_info_frame, text="Amount of item:", width=label_width, **text_styling)
...
#intial display for hired items instantly after items have been hired 
...
def refund_section():
    ...
    refund_info_frame = tk.LabelFrame(refund_frame, text="Refund Information", padx=80, pady=80, bg='#2A6543', fg="white")
    ...
    item_label = tk.Label(refund_info_frame, text="Item to Refund:", width=label_width, **text_styling)
    item_label.grid(row=2, column=0)
