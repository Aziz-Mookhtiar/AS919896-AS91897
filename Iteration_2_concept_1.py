...
def quit_confirmation():
    ...
    quit_window.configure(bg="#0000FF") 

    quit_frame = tk.Frame(quit_window, bg="#0000FF")  
    quit_frame.pack()

    confirmation = tk.Label(quit_frame, text="Are you sure you want to quit?", fg='#ADD8E6', bg="#0000FF")  
    ...
    button_styling = {"bg": "#afeeee", "fg": "#000000", "activebackground": "#7fffd4", "activeforeground": "#008b8b",
...
def display_receipt(receipt_number, first_name, last_name, item, amount):
    ...
    receipt_window.configure(bg='#0000FF')  

    receipt_frame = tk.Frame(receipt_window, bg='#0000FF') 
    receipt_frame.pack(padx=20, pady=20)

    tk.Label(receipt_frame, text=f"Receipt Number: {receipt_number}", fg='#ADD8E6', bg='#0000FF').pack(pady=5)  
    tk.Label(receipt_frame, text=f"First Name: {first_name}", fg='#ADD8E6', bg='#0000FF').pack(pady=5)  
    tk.Label(receipt_frame, text=f"Last Name: {last_name}", fg='#ADD8E6', bg='#0000FF').pack(pady=5)  
    tk.Label(receipt_frame, text=f"Item Chosen: {item}", fg='#ADD8E6', bg='#0000FF').pack(pady=5)  
    tk.Label(receipt_frame, text=f"Amount: {amount}", fg='#ADD8E6', bg='#0000FF').pack(pady=5)  

    return_button = tk.Button(receipt_frame, text="Back", command=receipt_window.destroy, **button_styling)
...
def hire_window():
    ...
    hire_window.configure(bg='#0000FF')  

    button_styling = {"bg": "#afeeee", "fg": "#000000", "activebackground": "#7fffd4", "activeforeground": "#008b8b",
                      "font": ("Comic Sans MS", 10, "bold")}

    text_styling = {"bg": "#0000FF", "fg": "#000000", "font": ("Comic San MS", 12, "bold")}  
    ...
    user_info_frame = tk.LabelFrame(hire_frame, text="Hire Information", padx=80, pady=80, fg="#ADD8E6", bg="#0000FF")  
    ...
    amount_spinbox = tk.Spinbox(user_info_frame, from_=1, to=500, width=entry_width)
    amount_spinbox.grid(row=4, column=1)

    # Button to save input data
    save_button = tk.Button(user_info_frame, text="Hire Item", command=input_data, padx=20, width=10, **button_styling)
    save_button.grid(row=5, column=0, pady=10)

    display_receipt_button = tk.Button(user_info_frame, text="Display Hired Items", command=display_hired_items, padx=20, width=12, **button_styling)
    display_receipt_button.grid(row=5, column=1, pady=10)

    back_button = tk.Button(user_info_frame, text="Back", command=hire_window.destroy, padx=22, width=10, **button_styling)
...
def display_hired_items():
    ...
    display_window.configure(bg='#0000FF')  
    display_window.geometry("900x290")

    display_frame = tk.Frame(display_window, bg='#0000FF')  
    ...
    for item in hired_items:
        tk.Label(display_frame,
                 ...
                 fg='#ADD8E6', bg='#0000FF').pack(pady=5)  
...
def refund_section():
    ...
    refund_window.configure(bg='#0000FF')  

    refund_frame = tk.Frame(refund_window)
    refund_frame.pack()

    refund_info_frame = tk.LabelFrame(refund_frame, text="Refund Information", padx=80, pady=80, bg='#0000FF', fg="#ADD8E6") 
    refund_info_frame.grid(row=0, column=0)

    label_width = 15
    entry_width = 20

    text_styling = {"bg": "#0000FF", "fg": "#ADD8E6", "font": ("Comic San MS", 12, "bold")}  
    ...
    display_receipt_button = tk.Button(refund_info_frame, text="Display Hired Items", command=display_hired_items, padx=20, width=12, **button_styling)
...
main_window.configure(bg='#0000FF')  
